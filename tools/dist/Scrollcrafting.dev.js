...
function callFetchScrolls() {
  ...
  fetch('https://api.idlescape.xyz/scrolls').then(function (response) {
    return response.json();
  }).then(function (json) {
    return traitementDataScrolls(json);
  });
} //-------------------------------------------
//Calculs
...
function traitementDataScrolls(json) {
  finalResultsScrolls = [];
  json.scrolls.forEach(function (e) {
    var totalPrice = 0;
    var compos = [];
    var mindRune = false;
    e.resources[0].forEach(function (c) {
...
function populateScrolls(craftName, sortingLevel) {
  ...
  finalResultsScrolls.forEach(function (e) {
    ...
    document.getElementById("Scrolls").getElementsByClassName("table")[0].tBodies[0].innerHTML += "<tr><th scope=\"row\">" + i + "</th><td><a class=\"btn btn-primary\" data-toggle=\"collapse\" href=\"#collapseScrolls" + i + "\" role=\"button\" aria-expanded=\"false\" aria-controls=\"collapseScrolls" + i + "\"><i class=\"glyphicon glyphicon-triangle-right\"></i>\t " + e.name + "</a>" + "<div class=\"collapse\" id=\"collapseScrolls" + i + "\"><div class=\"card card-body\">" + generateComposHtml(e.compos) + "</div></div>" + "</td><td>" + e.level + "</td><td>" + e.mindRune + "</td><td>" + millionFormate(e.CraftingPrice) + "</td><td>" + millionFormate(e.MarketPrice) + "</td><td class=\"" + (e.Benefits > 0 ? "positive" : "negative") + "\"" + "><b>" + millionFormate(e.Benefits) + "</b>" + "</td><td>" + millionFormate(e.exp) + "</td><td class=\"" + (e.ppxBenef > 0 ? "positive" : "negative") + "\"" + "><b>" + millionFormate(e.ppxBenef) + "</b>" + //"</td><td>" + e.prix_1xp +
    "</td></tr>"; //console.log(e.name + " -> Prix craft: " + e.prix + " | exp : " + e.exp + " | prix 1xp : " + e.prix_1xp);
