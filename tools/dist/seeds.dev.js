...
function callFetchSeeds() {
  ...
  fetch('https://api.idlescape.xyz/farming').then(function (response) {
    return response.json();
  }).then(function (json) {
    return traitementDataSeeds(json);
  });
} //-------------------------------------------
//Calculs
...
function populateSeeds() {
  ...
  finalResultsSeeds.forEach(function (e) {
    document.getElementById("Seeds").getElementsByClassName("table")[0].tBodies[0].innerHTML += "<tr><th scope=\"row\">" + i + "</th><td>" + e.name + "</td><td>" + e.prix + "</td><td>" + e.exp + "</td><td>" + e.minutes + "</td><td>" + e.width + "x" + e.height + "</td><td>" + e.xpMinute + "</td><td>" + e.prix_1xp + "</td></tr>"; //console.log(e.name + " -> Prix market: " + e.prix + " | exp total : " + e.exp + " | " + e.width + "x" + e.height + " | xp/bloc/min : " + e.xpMinute + " | prix 1xp : " + e.prix_1xp);
