...
function callFetchExp() {
  ...
  fetch('https://api.idlescape.xyz/crafting').then(function (response) {
    return response.json();
  }).then(function (json) {
    return traitementDataExp(json);
  });
} //-------------------------------------------
//Calculs
...
function traitementDataExp(json) {
  finalResultsExp = [];
  json.crafts.forEach(function (e) {
    var totalPrice = 0;
    var nameCompos = "";
    var i, j;
    ...
    for (i = 0; i < e.resources.length; i++) {
      ...
      for (j = 0; j < e.resources[i].length; j++) {
        ...
        compos.push({
          ...
          "quantity": e.resources[i][j].quantity
        });
      }
...
function populateExp(craftName, sortingLevel) {
  ...
  finalResultsExp.forEach(function (e) {
    ...
    document.getElementById("Exp").getElementsByClassName("table")[0].tBodies[0].innerHTML += "<tr><th scope=\"row\">" + i + "</th><td><a class=\"btn btn-primary\" data-toggle=\"collapse\" href=\"#collapseExp" + i + "\" role=\"button\" aria-expanded=\"false\" aria-controls=\"collapseExp" + i + "\"><i class=\"glyphicon glyphicon-triangle-right\"></i>\t " + e.name + "</a>" + "<div class=\"collapse\" id=\"collapseExp" + i + "\"><div class=\"card card-body\">" + generateComposHtml(e.compos) + "</div></div>" + "</td><td>" + e.level + "</td><td>" + millionFormate(e.CraftingPrice) + "</td><td>" + millionFormate(e.MarketPrice) + "</td><td class=\"" + (e.Benefits > 0 ? "positive" : "negative") + "\"" + "><b>" + millionFormate(e.Benefits) + "</b>" + "</td><td>" + millionFormate(e.exp) + "</td><td class=\"" + (e.ppxBenef > 0 ? "positive" : "negative") + "\"" + "><b>" + millionFormate(e.ppxBenef) + "</b>" + //"</td><td>" + e.prix_1xp +
    "</td></tr>"; //console.log(e.name + " -> Prix : " + e.prix + " | exp : " + e.exp + " | prix 1xp : " + e.prix_1xp);
